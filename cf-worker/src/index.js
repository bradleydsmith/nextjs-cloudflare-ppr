...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      const isNextStateTree =
        request.headers.get("next-router-state-tree") != null ? true : false;
      const isNextRouterPrefetch =
        request.headers.get("next-router-prefetch") === "1" ? true : false;

      if (
        request.method === "GET" &&
        (isNextRouterPrefetch || !isNextStateTree) &&
        route
      ) {
        if (isRSC) {
